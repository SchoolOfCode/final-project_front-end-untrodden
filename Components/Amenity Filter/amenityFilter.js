...
export default function AmenityFilter({ handleChange, value }) {
  let amenities = [
    ...
    {
      value: "amenities_public_transport_good",
      label: "Public transport good",
    },
        ...
        <Select
          ...
          renderValue={(selected) => {
            ...
            for (let i = 0; i < selectedAms.length; i++) {
              ...
              selectedAms[i] =
                selectedAms[i].charAt(0).toUpperCase() +
                selectedAms[i].slice(1);
            }
        ...
        >
          {amenities.map((amen) => (
            <MenuItem key={amen.value} value={amen.value}>
              ...
              <Checkbox
                style={{ color: "white" }}
                checked={value.indexOf(amen.value) > -1}
              />
            </MenuItem>
          ))}
        </Select>
